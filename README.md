...
这是一个尝试性的作品，它无法比拟微信自带的截屏翻译功能，调用本地模型也是一个华而不实的设计，唯一能够拿得出手的只是完全离线和稍长文本的翻译，而这两个功能取决于你的本地模型，很明显，这个作品没有达到我的预期，只能算个小玩具，不过欢迎你的使用。
...
3. 在 Settings 里选好模型并保存。运行 `ollama list` 可查看模型名称；默认的 `deepseek-r1:8b` 可以换成自己的模型。Ollama 地址通常保持 `http://127.0.0.1:11434` 即可。
